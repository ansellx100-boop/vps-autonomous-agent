...
- `{"type": "garmin_analyze", "days": 30}` — **анализ тренировок Garmin Connect** за период (нужны `GARMIN_EMAIL` и `GARMIN_PASSWORD` в `.env`, либо mock-режим);
...
### Garmin: анализ тренировок из аккаунта Garmin Connect

1. В `.env` задайте:
   - `GARMIN_EMAIL=ваш_email`
   - `GARMIN_PASSWORD=ваш_пароль`
2. Запустите локальный анализ:
   - `npm run garmin-analyze`
   - или за свой период: `npm run garmin-analyze -- --days 21`
3. Без реального логина можно проверить на фикстуре:
   - `npm run garmin-analyze -- --mock --mock-file tests/fixtures/garmin-activities.sample.json`
4. Дополнительно для стабильного живого доступа:
   - кэш токенов (по умолчанию): `GARMIN_TOKEN_DIR=data/garmin-tokens`
   - принудительный новый логин: `npm run garmin-analyze -- --force-login`
   - отключить кэш токенов: `npm run garmin-analyze -- --disable-token-cache`
   - ретраи при 429: `npm run garmin-analyze -- --retry-attempts 4 --retry-base-ms 30000`
   - данные сна (вкл/выкл): `--include-sleep` / `--no-sleep`
   - глубина истории сна: `--sleep-days 30` (ограничивается `GARMIN_MAX_SLEEP_DAYS`, по умолчанию 365)
   - mock-файл сна: `--sleep-mock-file tests/fixtures/garmin-sleep.sample.json`

Что делает анализ:
- загружает тренировки из Garmin Connect;
- считает метрики: объём, время, калории, пульс, темп, мощность, тренд 7 vs 7 дней;
- анализирует динамику пульса/мощности (7 vs 7) и отношение HR/Power;
- анализирует качество сна: средний sleep score, длительность, HRV, RHR и тренд 7 vs 7;
- формирует итоговый отчёт с базовыми рекомендациями и AI-разбором (если задан `OPENAI_API_KEY`).

### Веб-интерфейс Garmin Analytics

1. Запустите сервис в webhook-режиме:
   - `MODE=webhook PORT=3030 npm start`
2. Откройте в браузере:
   - `http://localhost:3030/garmin-ui`
3. Введите Garmin email/password, период и опции сна/AI, нажмите **«Запустить анализ»**.

Страница покажет:
- текстовый отчёт;
- структурированные метрики в JSON (включая блоки `hrPowerDynamics` и `sleepQuality`).
